...
const preview = {status:'stopped',log:'',command:config.command || '',port:config.port || 3000,managed:false};
...
const cleanEnvironment = () => ({PATH:process.env.PATH,HOME:process.env.HOME || '/root',LANG:'C.UTF-8',DISPLAY:':99',TERM:'xterm-256color',PORT:String(preview.port),SPARKLES_WORKSPACE_DIR:directory});
...
async function status() {
  if (preview.status === 'starting' || preview.status === 'ready') preview.status = await probe(preview.port) ? 'ready' : 'starting';
...
function defaultCommand() {
  if (existsSync(directory + '/package.json')) {
    const project = JSON.parse(readFileSync(directory + '/package.json','utf8'));
    const manager = existsSync(directory + '/pnpm-lock.yaml') ? 'pnpm' : existsSync(directory + '/yarn.lock') ? 'yarn' : 'npm';
    const script = project.scripts?.dev || project.scripts?.start;
    if (script) {
      if (!config.port) preview.port = Number(/--port(?:=|\s+)(\d+)/.exec(script)?.[1] || (script.includes('vite') ? 5173 : 3000));
      const install = manager === 'npm' && existsSync(directory + '/package-lock.json') ? 'npm ci' : manager + ' install';
      return '([ -d node_modules ] || ' + install + ') && ' + manager + ' run ' + (project.scripts.dev ? 'dev' : 'start');
    }
  }
...
async function previewStart(command, port) {
  ...
  if (preview.status === 'starting' || preview.status === 'ready') throw new Error('Stop the preview before changing its command.');
  preview.command = command;
  preview.port = port;
  preview.log = '';
  preview.status = 'starting';
  writeFileSync(configFile + '.tmp',JSON.stringify({command,port}),{mode:0o600});
  renameSync(configFile + '.tmp',configFile);
  if (await probe(port)) { preview.status = 'ready'; preview.managed = false; preview.log = 'Connected to the server already running on port ' + port + '.'; return; }
...
function desktopStart() {
  ...
  launch('desktop','bash',['-c',[
    ...
    'chromium --no-sandbox --disable-dev-shm-usage --no-first-run --start-maximized --user-data-dir=/tmp/sparkles-chromium about:blank &',
...
createServer(async (request,response) => {
  try {
    ...
    if (command.kind === 'preview-start') await previewStart(command.command,command.port);
    else if (command.kind === 'preview-stop') { preview.status = 'stopped'; revoke('preview'); stop('preview'); }
